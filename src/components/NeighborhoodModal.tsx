...
import { X, MapPin, ShoppingBag, Utensils, Landmark, ArrowRight, Sparkles, Navigation } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md overflow-y-auto animate-fadeIn">
      <div className="relative w-full max-w-4xl bg-[#081222] border border-white/15 rounded-3xl overflow-hidden shadow-[0_25px_70px_rgba(0,0,0,0.8)] flex flex-col max-h-[85vh] my-auto">
        ...
        {/* Content Body - POPPING VALUE PROP CARDS */}
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {/* Hero Banner */}
          <div className="relative rounded-2xl overflow-hidden border border-cyan-500/30 h-48 bg-slate-900 shadow-xl">
            <img
            ...
            <div className="absolute bottom-4 left-5 right-5 flex items-center justify-between">
              <div>
                <div className="text-white font-display font-bold text-xl drop-shadow-md">Yorkville / Bloor Luxury District</div>
                <div className="text-xs text-slate-300 font-light">The epicentre of Canadian wealth and culture</div>
              </div>
          ...
          {/* Landmarks Category Value Prop Cards with High Pop */}
                  ...
                  <div className="space-y-2.5">
                    {sec.items.map((it, i) => (
                      <div
                        ...
                        className="flex items-start justify-between gap-4 p-3.5 rounded-xl bg-[#091f3d]/70 hover:bg-[#0f2c54] border border-cyan-500/25 hover:border-cyan-400/60 shadow-sm transition-all hover:scale-[1.01] cursor-pointer group"
                      >
                        <div className="space-y-1">
                          <div className="text-sm font-bold text-white group-hover:text-cyan-200 transition-colors flex items-center gap-2">
                            <span>{it.name}</span>
                            <Navigation className="w-3 h-3 text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                          </div>
